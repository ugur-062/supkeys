 ...
 * sunucudaki sayımı kullanıyordu ve o sayım ilan TİPİNİ süzmüyordu — satın
 * alma tarafında verilen (SATIS ilanına) teklifler satış panosuna sayılıyordu;
...
/** Satış tarafı = ALIM ilanlarına verdiğim teklifler; satınalma = SATIS ilanlarına. */
export function selectActiveOffers(bids: MyBid[], type: "ALIM" | "SATIS"): MyBid[] {
  return bids.filter(
    (b) =>
      b.listing.type === type &&
      b.status === "SUBMITTED" &&
      OFFER_OPEN_LISTING.has(b.listing.status),
  );
}

/** Kazandığım işler — KISMİ kazanım DAHİL (etiket bunu söyler). */
export function selectWonOffers(bids: MyBid[], type: "ALIM" | "SATIS"): MyBid[] {
  return bids.filter(
    (b) =>
      b.listing.type === type &&

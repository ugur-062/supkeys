...
export function SetupCard({ value, onChange, onDone, saving }: { value: RequestDefaults; onChange: (v: RequestDefaults) => void; onDone: () => void; saving: boolean }) {
  const t = useTranslations("web.panel.requests.setupCard");
  return (
    <div className="rounded-2xl border border-blue-200 bg-blue-50/60 p-5">
      <p className="text-sm font-semibold text-blue-950">{t("ilkTalebinizUcKisaSoru")}</p>
      <p className="mt-0.5 text-xs text-blue-900/80">{t("bunlariBirKezCevaplayinSonraki")}</p>
      ...
      >
        {saving ? t("kaydediliyor") : t("kaydetVeDevamEt")}
      </button>
    </div>
  );
}

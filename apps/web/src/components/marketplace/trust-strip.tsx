 ...
 */
const ITEM_ICONS = [CheckBadgeIcon, LockClosedIcon, BanknotesIcon, ShieldCheckIcon] as const;
const ITEM_KEYS = ["verified", "sealed", "noFee", "kvkk"] as const;

export function TrustStrip() {
  const t = useTranslations("web.marketing.trustStrip");
  const ITEMS = ITEM_KEYS.map((k, i) => ({ icon: ITEM_ICONS[i]!, title: t(`${k}Title`), body: t(`${k}Body`) }));
  return (
    <section aria-label={t("label")} className="border-y border-zinc-950/5 bg-zinc-50">

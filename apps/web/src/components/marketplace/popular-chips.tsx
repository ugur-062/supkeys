...
export function PopularChips({
  ...
  allHref = MARKETPLACE_ROUTES.products,
}: {
  items: { id: string; name: string; count: number }[];
  hrefFor?: (c: { id: string; name: string; count: number }) => string;
  allHref?: string;
}) {
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
      ...
      <ul className="mt-4 flex flex-wrap gap-2">
        {items.map((c) => (
          <li key={c.id}>
            <Link href={hrefFor(c)} className="inline-flex items-center gap-1.5 rounded-full bg-zinc-100 px-3 py-1.5 text-sm text-zinc-700 transition hover:bg-zinc-200">
              {c.name}

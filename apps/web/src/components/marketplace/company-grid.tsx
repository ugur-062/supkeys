...
export function CompanyGrid<T extends PublicDirectoryCard>({
  companies,
  heading = "Rothern'daki firmalar",
  hrefFor,
  allHref = MARKETPLACE_ROUTES.companies,
}: {
  companies: T[];
  heading?: string;
  /**
   * Kart hedefi — panel firma sayfasına gider (public profil değil).
   * Jenerik: panel dizini `rothernId` taşır, public kart taşımaz; tip
   * çağırandan gelsin ki panelde `as` gerekmesin.
   */
  hrefFor?: (c: T) => string;
  allHref?: string;
}) {
    ...
    <section className="mx-auto max-w-7xl px-6 py-14 lg:px-8">
      ...
      <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {companies.slice(0, 6).map((c) => (
          <CompanyCard key={c.slug} company={c} href={hrefFor?.(c)} />
        ))}
      </div>
    </section>
  );

...
import { QUICK_TO_WIZARD_KEY, readSession } from "@/lib/tenders/quick-draft";
import { Sparkles } from "lucide-react";
import { useSearchParams } from "next/navigation";
import { useEffect, useState } from "react";

/**
 * DETAYLI TALEP — mevcut sihirbaz (2026-09-09: varsayılan giriş artık hızlı
 * kart; bu sayfa kopya/AI belge/şablon ve "Detaylı ayarlar" için).
 */
export default function DetayliTalepPage() {
  ...
  /** Hızlı karttan "Detaylı ayarlar" — girilenler aynen taşınır. */
  const fromQuick = params.get("kaynak") === "hizli";
  const [quickSeed, setQuickSeed] = useState<TenderFormData | null>(null);
  useEffect(() => {
    if (!fromQuick) return;
    const v = readSession<TenderFormData>(QUICK_TO_WIZARD_KEY, true);
    if (v) setQuickSeed(v);
  }, [fromQuick]);
  ...
  if (quickSeed) {
    return <TenderWizard key="quick-seed" initialValues={quickSeed} />;
  }

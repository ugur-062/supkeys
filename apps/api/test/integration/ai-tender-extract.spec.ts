...
// ── El yapımı minimal PDF üretici (harici PDF-yazma bağımlılığı yok) ─────────
// pageTexts[i] null → o sayfada METİN KATMANI YOK (taranmış sayfa benzetimi).
function makeSimplePdf(pageTexts: (string | null)[]): Buffer {
  ...
  for (const t of pageTexts) {
    ...
    // Uzun metni sayfa sınırları içinde satırlara böl — pdfjs sayfa dışına
    // taşan glifleri metin çıkarımında kırpabiliyor.

...
import { useSupplierApplications } from "@/hooks/use-supplier-applications";
import { APPLICATION_STATUS_ORDER } from "@/lib/applications/status";
import type { ApplicationStatus } from "@/lib/applications/types";
import { useRouter, useSearchParams } from "next/navigation";
import { useCallback, useMemo, useState } from "react";
import { SupplierApplicationsTable } from "./applications-table";
import { SupplierDetailDrawer } from "./detail-drawer";
import { FiltersBar } from "./filters-bar";
import { Pagination } from "./pagination";
import { SupplierStatsCards } from "./stats-cards";
...
function SupplierApplicationsContent() {
  ...
  const list = useSupplierApplications(queryParams);
  ...
  const updateUrl = useCallback(
    (next: {
      ...
      router.replace(
        qs
          ? `/admin/supplier-applications?${qs}`
      ...
      <div className="space-y-5 max-w-6xl">
        <div className="space-y-1">
          ...
          <p className="text-admin-text-muted">
            Self-register veya alıcı daveti üzerinden gelen tedarikçi
            başvurularını buradan inceleyin.
          </p>
        </div>
...
export function SupplierApplicationsView() {
  return (
    <RequireAdminAuth>
      <SupplierApplicationsContent />
    </RequireAdminAuth>
  );
}

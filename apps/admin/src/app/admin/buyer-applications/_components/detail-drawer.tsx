...
import { useBuyerApplicationDetail } from "@/hooks/use-buyer-applications";
...
import { ApproveBuyerDialog } from "./approve-dialog";
import { RejectBuyerModal } from "./reject-modal";
import { TaxCertModal } from "./tax-cert-modal";
...
export function BuyerDetailDrawer({ id, onClose }: DetailDrawerProps) {
  const open = !!id;
  const detail = useBuyerApplicationDetail(id);
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
            ...
            {item && (
              <>
                {/* Demo davet banner — alıcı her zaman demo davetli olmalı */}
                {item.fromDemoRequest ? (
                  <div className="rounded-xl border border-warning-200 bg-warning-50 p-4">
                    <div className="flex items-start gap-3">
                      <Sparkles className="w-5 h-5 text-warning-600 shrink-0 mt-0.5" />
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-warning-700">
                          🎯 Demo Davetli Başvuru
                        </p>
                        <p className="text-xs text-warning-600/90 mt-1">
                          Bu başvuru &ldquo;
                          <strong>
                            {item.fromDemoRequest.companyName}
                          </strong>
                          &rdquo; demo talebinden geldi.{" "}
                          <Link
                    ...
                    {item.tenant && (
                      <InfoRow label="Tenant">
                        <span className="inline-flex items-center gap-1.5">
                          {item.tenant.name}
                          <span className="text-xs text-admin-text-muted font-mono">
                            ({item.tenant.slug})
                          </span>
                        </span>
                      </InfoRow>
            ...
            <footer className="px-5 py-4 border-t border-admin-border bg-admin-surface shrink-0">
              <div className="rounded-lg bg-warning-50 border border-warning-200 px-3 py-2.5 flex items-start gap-2.5">
                ...
                <p className="text-xs text-warning-700 leading-relaxed">
                  Kullanıcı henüz e-posta adresini doğrulamadı. Doğrulama
      ...
      {item && (
        <>
          <ApproveBuyerDialog
            ...
            fromDemoCompanyName={item.fromDemoRequest?.companyName ?? null}
            open={approveOpen}

...
- **INV-MT-5 (GÜVENLİK AĞI — 24 tablo LOKAL-KANITLI, PROD'DA KAPALI)** — Postgres RLS bir **güvenlik ağı (backstop)** olarak eklendi: servis katmanında scope'suz yazılmış tek bir sorguyu DB'de yakalar. **RLS servis disiplinini İKAME ETMEZ** — birincil tenant kapısı INV-MT-1..4 (servis katmanı) olarak KALIR. **Durum (2026-07-21):** plumbing (ALS + Prisma extension + kısıtlı rol + bypass client) + **24 tablo gerçek policy** (`rls-isolation.spec` izolasyon-kanıtlı, full-suite yeşil). **Prod'da HÂLÂ KAPALI** (`RLS_ENABLED` set değil + owner rol → policy bypass); aktivasyon ayrı adım (`docs/launch-checklist.md § RLS aktivasyon`). Plan/kapsam: `docs/rls-plan.md`.
  - **HANGİ TABLO RLS ALIR — İLKE (2026-07-21):** RLS'in getirisi, tablonun tenant-kapısının NASIL enforce edildiğine bağlıdır:
    - **Tenant-scoped BASİT** (`companyId=current` tek koşul: addresses, bank, templates, approvals…) → **RLS backstop AL.** Ucuz, net, scope'suz sorguyu yakalar.
    - **İki-taraflı / kapalı-zarf** (order/bid/connection/message/invitation: `IN(a,b)` veya EXISTS-parent) → **RLS AL.** Görünürlük satır-şekilli; backstop değer katar.
    - **DAĞINIK guard** (görünürlük çok yere serpilmiş imperatif kontrol; ör. orders `assertOrderRole` + transition guard'ları) → **RLS policy GERÇEK backstop.** Tek bir unutulmuş guard'ı DB yakalar. *(orders'a policy AL kararının gerekçesi budur.)*
    - **TEK-HELPER görünürlük** (kural tek, test-edilmiş, tek-kaynak servis helper'ında; ör. listings `listing-visibility.ts`/INV-VIS-1: owner OR PUBLIC OR invited OR CONNECTIONS-bağlı) → **PERMISSIVE bırak.** RLS marjinal (guard zaten dağınık değil, drift riski düşük) ama maliyet yüksek (4-yollu EXISTS, hot-feed perf, cross-tenant okuma audit, all-or-nothing). Getiri < bedel → force-fit ETME.
    - **Directory/shared** (companies/company_users/notifications/company_user_invitations: cross-tenant OKUNUR/YAZILIR by design) → **PERMISSIVE.** Strict policy özelliği kırar.
  - *Geçmiş belirtiler:* bu turlardaki durum-yarışları (INV-SM-1: #1/#5/#11) ve fail-open admin authz (#12) hep ağ eksikliğinin belirtileriydi — nokta-atışı kapatıldı, artık DB-seviyesi ağ da 24 tabloda mevcut (lokal).
  ...
  - **Prod'da hâlâ inert (aktivasyon bekliyor):** Prisma prod'da tabloların **sahibi** olan owner rolüyle bağlanır (`DATABASE_URL`); tablo sahibi RLS'i baypas eder → prod'da policy'ler yazılı ama etkisiz. **Lokalde AKTİF** (`rls-isolation.spec`: kısıtlı `rothern_app` rolü + `RLS_ENABLED=true` → policy'ler enforce, izolasyon kanıtlı). Aktivasyon = ana `DATABASE_URL`'i kısıtlı role çevir + `RLS_ENABLED=true` (ayrı adım, `docs/launch-checklist.md`).
  ...
  - **Aşamalı yol (İZLENDİ ✅):** plumbing → permissive no-op → 9 temiz direct → transitif → iki-taraflı → kapalı-zarf → orders. Admin/public-katalog/cron için **bypass client** (owner rol, RLS-extension'sız). `WITH CHECK` (yazma) `USING`'den (okuma) sonra. **24 tablo tamam; listings + 4 directory bilinçli permissive (yukarıdaki İLKE).**
  - **Kalan:** yalnız **prod aktivasyon** (kısıtlı rol provision + pooler custom-rol auth doğrulama + `DATABASE_URL` rol-geçişi + kill-switch) — `docs/launch-checklist.md § RLS aktivasyon`. Launch-blocker DEĞİL.
...
- **A1 — Satıcı iptal talebi + `DISPUTED`:** satıcı yalnız `ACCEPTED`'te iptal TALEP eder (order alanı `cancelRequestedAt`, durum değişmez); alıcı onaylar → `CANCELLED` ya da reddeder → `DISPUTED` (ACCEPTED'a DÖNMEZ). `DISPUTED` iki-yönlü çıkış: satıcı `ship` (→`IN_DELIVERY`) VEYA alıcı onay (→`CANCELLED`). **Otomatik onay YOK.** Ayrı tablo yok — tek gerekçe + sonucu order statüsü; tam kanıt `audit_logs`'ta (platform hakem değil, kaydeder). Açık talep = `status===ACCEPTED && cancelRequestedAt!=null`. Yan etki: `DISPUTED`'da vade cron (DELIVERED-filtreli) çalışmaz ve revizyon reddedilir. **Ödeme/LC penceresi A1-DISPUTED'ta AÇIK** (`isPaymentOpen` + `lcMarkOpened`/`lcMarkAccepted` `defectNotifiedAt==null` iken DISPUTED'ı kabul eder) — peşin eşiği/LC kabulü sevk çıkışının ÖN KOŞULU olduğundan pencere kapalıyken iki-yönlü çıkış tek yönlü kalıyordu (denetim 2026-08-23 Parça 3 #2). Ayıp-DISPUTED'ta ikisi de KAPALI.
...
- **INV-SM-4 (REVİZE — yaşam döngüsü ayrımı)** — Sipariş yaşam döngüsü (operasyonel: mal geldi/kabul edildi mi) ile ödeme yaşam döngüsü (finansal: borç kapandı mı) AYRIDIR. `COMPLETED` = alıcının malı KABULÜ (`complete()`, DELIVERED→COMPLETED); **ödeme şartı YOK** — vadeli/açık-hesap/çek/senet/mal-mukabili siparişte alıcı malı kabul edip tamamlar, borç açık kalabilir. Ödeme durumu **türetilir** (`paymentSettled = confirmedSum >= total`, `isFullyPaid`; yeni alan yok). Ödeme→durum kaplini KALDIRILDI: Madde 17'den sonra `receive()` IN_DELIVERY→**COMPLETED** oto-tamamlar (ödeme yine bağımsız izlenir), `paymentDecision`/`lcMarkPaid` ödeme onayı sipariş durumunu DEĞİŞTİRMEZ. Vade cron (`sendDuePaymentReminders`) `DELIVERED`+`COMPLETED` izler (DISPUTED hariç — saat durur). Liste/KPI "Ödeme Bekleyen" status yerine `paymentSettled`'dan. (Eski INV-SM-4 "COMPLETED tam ödeme ister" GEÇERSİZ.)
...
- **INV-ENT-2** — Yeni ilan işi (ilan aç/yayınla/yeni tur/davet) `PAKET` üyelik gerektirir; kontrol `assertPaidForNewListingWork` (`company-listings.service.ts`) veya `CompanyPaidTierGuard` ile yapılır. `STANDARD` üye yalnız mevcut satın alma taleplerine teklif verebilir.
...
- **INV-VIS-1 (görünürlük tek kaynak)** — "Bir izleyici bir firmanın açık satın alma talebini görür mü?" kuralı TEK KAYNAKTAN gelir: **PUBLIC herkese; CONNECTIONS yalnız AKTİF bağlıya; PRIVATE yalnız o ilana DAVETLİYE** (davet her görünürlüğü aşar). "Bağlı olmak ≠ davetli olmak" — bağlı firma PRIVATE (davet-only) satın alma talebini GÖRMEZ. Helper: `common/company/listing-visibility.ts` — `isListingVisibleToViewer` (tekil/boolean, `getOne` 3 çağrı yeri) + `visibleOwnerListingWhere` (tek-firma findMany `where`, `getProfile`). `sellerTenders` çok-firma+ülke bileşik varyantı (aynı çekirdek).
  - *Geçmiş (F-CONN-1, 2026-07-17):* `getProfile` bağlı firmaya `connected ? {}` (görünürlük filtresi YOK) uyguluyordu → PRIVATE satın alma taleplerin varlığı+başlığı bağlı-ama-davetsiz firmaya sızıyordu (getOne 404 verirken). Kardeş-yol tek-helper'a bağlandı. Kanıt: `connections.spec.ts` (F-CONN-1), `visibility-matrix.spec.ts` (getOne).
...
- **INV-MT-5 (BÜYÜK ÖLÇÜDE SAĞLANDI — yalnız prod aktivasyon kaldı)** — Postgres RLS güvenlik ağı **24 tabloda lokal-kanıtlı** (plumbing + policy + izolasyon testi); prod'da HÂLÂ KAPALI (owner rol + `RLS_ENABLED` yok → policy bypass). Yani prod'da henüz "yürürlükte" değil ama iş bitmiş, aktivasyon ayrı adım bekliyor. Tam durum + tablo-seçim İLKESİ + aktivasyon adımları için bölüm 1'e (INV-MT-5) + `docs/rls-plan.md` + `docs/launch-checklist.md`'ye bakınız.

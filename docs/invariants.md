...
- **INV-KYC-1** — Para-taahhüdü doğuran aksiyonlar firma KYC doğrulaması (`companyVerificationStatus === "VERIFIED"`) ister: teklif SUBMIT (`placeBid`, **taslak HARİÇ**), `award`, `awardByItem`, `publishListing`. Kontrol `assertVerified` (`company-listings.service.ts`, `assertPaidForNewListingWork` simetriği; `user.companyVerificationStatus` JWT'den her istekte TAZE okunur → doğrulanır doğrulanmaz açılır). Gezinme, keşif, davet kabul, TASLAK kaydetme ve sipariş-akışı SERBEST (funnel kırılmaz). Onay-zinciri finalizasyonu (`onAwardApproved`) gated DEĞİL — initiator zaten `award`/`awardByItem` çağrısında kapıdan geçti (INV-AZ-3 deseni). Kanıt/regresyon: `test/integration/kyc-gate.spec.ts`.

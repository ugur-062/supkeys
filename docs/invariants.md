...
- **INV-SM-4 (REVİZE — yaşam döngüsü ayrımı)** — Sipariş yaşam döngüsü (operasyonel: mal geldi/kabul edildi mi) ile ödeme yaşam döngüsü (finansal: borç kapandı mı) AYRIDIR. `COMPLETED` = alıcının malı KABULÜ (`complete()`, DELIVERED→COMPLETED); **ödeme şartı YOK** — vadeli/açık-hesap/çek/senet/mal-mukabili siparişte alıcı malı kabul edip tamamlar, borç açık kalabilir. Ödeme durumu **türetilir** (`paymentSettled = confirmedSum >= total`, `isFullyPaid`; yeni alan yok). Ödeme→durum kaplini KALDIRILDI: `receive()` her zaman `DELIVERED` (eski BEFORE_DELIVERY oto-COMPLETE gitti), `paymentDecision`/`lcMarkPaid` ödeme onayı sipariş durumunu DEĞİŞTİRMEZ. Vade cron (`sendDuePaymentReminders`) `DELIVERED`+`COMPLETED` izler (DISPUTED hariç — saat durur). Liste/KPI "Ödeme Bekleyen" status yerine `paymentSettled`'dan. (Eski INV-SM-4 "COMPLETED tam ödeme ister" GEÇERSİZ.)

...
1. [railway.app](https://railway.app) → **New Project** → **Deploy from GitHub repo** → `rothern`.
...
### `admin` servisi
`NEXT_PUBLIC_API_URL=https://api.rothern.com/api`, `NEXT_PUBLIC_SUPABASE_URL`,
`NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXT_PUBLIC_SITE_URL=https://admin.rothern.com`.
...
(`api.rothern.com` vb.). Railway sana bir **CNAME hedefi** verir (`xxx.up.railway.app`).

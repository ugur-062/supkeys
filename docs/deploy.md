# Supkeys — Üretim Dağıtımı (Coolify + Hetzner)
...
|----------|-----------|------|--------|
| API (NestJS) | `apps/api/Dockerfile` | 4000 | `api.supkeys.com` |
| Web (Next) | `apps/web/Dockerfile` | 3000 | `app.supkeys.com` |
| Admin (Next) | `apps/admin/Dockerfile` | 3001 | `admin.supkeys.com` |
...
Coolify → **Project** oluştur (ör. `supkeys`) → içine **3 ayrı Application**, hepsi
aynı repoyu gösterir, **Build Pack = Dockerfile**:

| Application | Dockerfile yolu | Base directory | Port |
|-------------|-----------------|----------------|------|
| `supkeys-api` | `apps/api/Dockerfile` | `/` (repo kökü) | 4000 |
| `supkeys-web` | `apps/web/Dockerfile` | `/` | 3000 |
| `supkeys-admin` | `apps/admin/Dockerfile` | `/` | 3001 |
...
- api → `https://api.supkeys.com`
- web → `https://app.supkeys.com`
- admin → `https://admin.supkeys.com`
...
### `supkeys-api` → **Environment Variables** (runtime)
...
`COOKIE_DOMAIN=.supkeys.com`, `EMAIL_*`, `RESEND_API_KEY`, `R2_*`, `SENTRY_*`,
`LOG_LEVEL`, ve İLK deploy için `RUN_SEED=true` + `INITIAL_ADMIN_*`.

### `supkeys-web` / `supkeys-admin` → **Build Variables (Build Args)**
...
`NEXT_PUBLIC_API_URL=https://api.supkeys.com/api`,
`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`,
`NEXT_PUBLIC_SITE_URL` (web→app.supkeys.com, admin→admin.supkeys.com).
...
1. **`supkeys-api`'yi deploy et.** Container başlarken entrypoint önce
...
4. **`supkeys-web` + `supkeys-admin`'i deploy et.** (Build args set olmalı.)
5. Uçtan uca doğrula:
   - `app.supkeys.com/login` → tenant giriş → cookie `sk_company` (HttpOnly, Secure, Domain=.supkeys.com)
   - `admin.supkeys.com/admin/login` → `INITIAL_ADMIN_*` ile giriş
...
- **Node ≥22.18 zorunlu:** `@supkeys/db` ham TS export eder; derlenmiş API runtime'da
...
- **Cookie/CORS:** cross-subdomain oturum yalnız gerçek `*.supkeys.com` + HTTPS'te tam
  çalışır (`COOKIE_DOMAIN=.supkeys.com` + `NODE_ENV=production` Secure). `localhost` mekanizmayı doğrular ama domain paylaşımını değil.
